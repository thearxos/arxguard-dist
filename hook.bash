...
# subprocess, external grep/sed, or per-command scanner process is used.
[[ $- == *i* ]] || return 0
...
# Load the native scanner once. The DEBUG trap remains only as the shell integration
# boundary; all threat detection runs inside the already-running Bash process.
if ! builtin type -t arxguard-scan >/dev/null 2>&1; then
  builtin enable -f "$ARXGUARD_LIB/arxguard-builtin.so" arxguard-scan 2>/dev/null || return 0
fi
_arxguard_scan() { arxguard-scan "$1"; }
